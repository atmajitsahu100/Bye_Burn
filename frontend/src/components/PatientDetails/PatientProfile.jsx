            ...
            <div className=' w-full h-[40%] p-3 flex flex-row items-center justify-between'>
                <div className=" w-1/2 h-full bg-slate-200 p-4 rounded-md hover:shadow-md">
                    <div className="grid grid-cols-2 gap-y-2">
                        {patientDetails && (
                            <>
                                <p className="text-lg text-gray-600"><span className="font-bold text-black">Patient Id:</span> {(shortenId(patientId))}</p>
                                <p className="text-lg text-gray-600"><span className="font-bold text-black">Patient Name:</span> {patientDetails.firstName} {patientDetails.lastName}</p>
                                <p className="text-lg text-gray-600"><span className="font-bold text-black">Age:</span> {patientDetails.age} </p>
                                <p className="text-lg text-gray-600"><span className="font-bold text-black">Gender:</span> {patientDetails.gender} </p>
                                <p className="text-lg text-gray-600"><span className="font-bold text-black">Weight:</span> {patientDetails.weight} </p>
                                <p className="text-lg text-gray-600"><span className="font-bold text-black">Contact:</span> {patientDetails.mobile} </p>
                            </>
                        )}
                    </div>
                </div>
                <div className='w-1/2 h-full p-3 m-2 rounded-md flex flex-row justify-between'>
                    <div className='flex flex-col'>
                        <div className=' w-full p-4 pt-2 flex flex-row'>
                            <label className="block mt-2 mr-2 text-md font-medium text-gray-900 dark:text-white " htmlFor="file_input">Upload</label>
